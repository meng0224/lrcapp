...
- **權限處理**：
  ...
  - Android 11+：檔案匯入與自訂輸出目錄都依賴 SAF 授權，不要求「所有檔案存取權」
- **輸出位置**：
...
3. （可選）點擊「選擇目錄」設定輸出資料夾（使用 SAF `OpenDocumentTree`），未設定則使用預設應用下載目錄
...
- **Android 11+ 權限策略**：目前檔案匯入與自訂輸出資料夾都依賴 SAF，不要求 `MANAGE_EXTERNAL_STORAGE`；預設輸出則使用 App-specific downloads。
